...
import {
  getPaymentIndex,
  updatePayment
} from '../db/queries/paymentRepository'
import { type Game } from '../db/interfaces'

export class CougarBot {
  private readonly playhq: PlayHqService
  ...
  public async getNextGame (): Promise<Game> {
    try {
      ...
      upcomingGame.whoIsPaying = parseInt(
        (await this.determineWhoIsPaying()) as string
      )
      ...
      console.error(error)
      throw new Error('Unable to get next game')
    }
  }
  ...
  private sortArrayWithIntegerAtTop (
    ...
    const player = players.find((p) => p.availableToPlay)
    if (player == null) {
    ...
    remainingPlayers.sort(
      (a: Player, b: Player): number =>
        Number(b.availableToPlay) - Number(a.availableToPlay)
    )
  ...
  async determineWhoIsPaying (): Promise<boolean | string> {
    // get list of players
    ...
    const sortedPlayers = this.sortArrayWithIntegerAtTop(
      players,
      paymentIndex
    )
    ...
    if (availablePlayerName != null) {
      ...
      await updatePayment(newPaymentIndex, { paymentIndex: 1 })
    }

    return (
      availablePlayerName != null ||
  ...
  private static extractGameSummary (
  ...
  createGameMessage (game: Game): string {
    return `Date: ${game.date.toDateString()}
            Game: ${game.homeTeam} vs ${game.awayTeam}
            Location: ${game.location} - Court ${game.courtNumber}
            Paying: ${game.whoIsPaying}`
  }
}
